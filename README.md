...
- *atlases/deterministic/*: The original Schaefer 1000 and Tian 54 atlases in NIfTI format (with txt label files) used for the atlas integration, as well as the combined atlas (with a csv file containing the integrated labels) used in many downstream analyses resulting from the integration process described in `03_atlas_integration.ipynb`.
...
### `09_LRG_community_detection.ipynb`
...
- `connectivity_matrices/`: contains utility functions (and main scripts if you wish to skip the notebooks) for the merging and aggregation of rfMRI timeseries in `04_merge_aggregate_rfMRI.ipynb` and for the aggregation of dMRI measures in `05_aggregate_dMRI.ipynb`. Also, contains functions and a main script for determining edge-wise associations for all functional and structural matrices with covariates in `06_edge_associations.ipynb`.
